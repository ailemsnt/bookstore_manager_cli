...
export class BookView extends ConsoleView {
  constructor(private readonly bookSrv: BookService, private readonly authorSrv: AuthorService)
  { 
    super(); 
  }
  ...
  private async findAllBooks(): Promise<void> {
    ...
    list.forEach((book) => {
      ...
      this.display(`${book.id} - #${book.codigo}: ${(book.titulo).toUpperCase()}`);
  ...
  private async createBook(): Promise <void> {
    ...
    while (true) {
      ...
      const authorExists = await this.authorSrv.findAuthorById(authorIdValidate);
      if (!authorExists) {            
        continue;
      }
    ...
    const bookCreated = await this.bookSrv.createBook({ titulo: bookDto.titulo, editora: bookDto.editora, edicao: bookDto.edicao, ano_publicacao: Number(bookDto.ano_publicacao), baixado: formatInChar(bookDto.baixado), codigo: bookDto.codigo, isbn: bookDto.isbn, 
    autores: authorsId
    });
  ...
  private async updateBook(): Promise<void> {
    ...
    const bookUpdated = await this.bookSrv.updateBook( {id: Number(idUpdate), titulo: bookUpdateDto.titulo, editora: bookUpdateDto.editora, edicao: bookUpdateDto.edicao, ano_publicacao: Number(bookUpdateDto.ano_publicacao), baixado: formatInChar(bookUpdateDto.baixado), autores: authorsIdUpdate });
