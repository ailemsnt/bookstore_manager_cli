...
export class BorrowView extends ConsoleView {
  ...
  constructor(private readonly borrowSrv: BorrowService, private readonly bookSrv: BookService, private readonly customerSrv: CustomerService )
  { 
    super(); 
  }  
  ...
  private async selectCustomer(): Promise<number> {
    while (true) {
      ...
      while (true) {
        ...
        const customerExists = await this.customerSrv.findCustomerById(customerIdValidate);
        if (!customerExists) {            
          continue;
        } 
  ...
  private async findBorrowById(): Promise<void> {
    ...
    this.display('');
    this.display(`Empréstimo ID: #${borrowById.id} Data empréstimo: ${formatDate(borrowById.data_emprestimo)}`);
    this.display(`Cliente: #${borrowById.cliente_id}: ${(borrowById.cliente_nome).toUpperCase()}\n`);
    this.display(`Livro(s):`);
    borrowById.livros.forEach((livro) => {       
      this.display(`#${livro.id} - ${livro.codigo}: ${(livro.titulo).toUpperCase()}`);
      ...
      this.display(`Data prevista devolução: ${formatDate(livro.data_prevista_devolucao)} • Devolução: ${livro.data_devolucao ? formatDate(livro.data_devolucao) : '- '}`);
  ...
  private async findBorrowBybook(): Promise<void> {
    ...
    borrowByBookId.forEach((borrow) => {
      this.display('----------');
      this.display(`Empréstimo ID: #${borrow.id} Data empréstimo: ${formatDate(borrow.data_emprestimo)}`);
  ...
  private async findBorrowByCustomer(): Promise<void> {
    ...
    const borrowByCustomerId = await this.borrowSrv.findBorrowFilter(this.filter);                
    ...
    borrowByCustomerId.forEach((borrow) => {
      ...
      borrow.livros.forEach((livro) => {
        ...
        this.display(`Autor(es): ${livro.autores.map((autor) => autor.nome).join(', ')}`);
        this.display(`Editora: ${livro.editora} • ${livro.edicao} • ${livro.ano_publicacao} • ISBN: ${livro.isbn}`); 
        this.display(`Data prevista devolução: ${formatDate(livro.data_prevista_devolucao)} • Status: ${livro.status}\n`);
      });
  ...
  private async findActiveBorrow(): Promise<void> {
    ...
    listBorrowOpened.forEach((borrow) => {
      ...
      borrow.livros.forEach((livro) => {
        ...
        this.display(`Autor(es): ${livro.autores.map((autor) => autor.nome).join(', ')}`);
        this.display(`Editora: ${livro.editora} • ${livro.edicao} • ${livro.ano_publicacao} • ISBN: ${livro.isbn} `);
        this.display(`Data prevista devolução: ${formatDate(livro.data_prevista_devolucao)} `);
        this.display(`Status: ${livro.status}\n`)});
    }); 
    this.display('');  
  }
  ...
  private async createBorrow(): Promise<void> {
    ...
    while (true) {
      ...
      if (booksId.length === 5) {
        this.display(`Atingido a quantidade máxima de 5 livros por empréstimo.`);
        break;
      }
    ...
    const borrowCreated = await this.borrowSrv.createBorrow({ cliente_id: Number(borrowDto.cliente_id)}, booksId, Number(this.userId));
  ...
  private async deleteBorrow(): Promise<void> {
    ...
    this.display('----------');
    this.display(`Empréstimo ID: #${returnById.id} Data empréstimo: ${formatDate(returnById.data_emprestimo)}`);
    this.display(`Cliente: #${returnById.cliente_id}: ${(returnById.cliente_nome).toUpperCase()}\n`);
    this.display('Livro(s):');
